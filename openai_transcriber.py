...
def run(on_transcript, audio_chunks, on_status=None):
    asyncio.run(_run(on_transcript, audio_chunks, on_status))


async def _run(on_transcript, audio_chunks, on_status):
    headers = {
        "Authorization": f"Bearer {config.OPENAI_API_KEY}",
        "OpenAI-Beta": "realtime=v1",
    }
    url = f"{_REALTIME_URL}?model={config.OPENAI_REALTIME_MODEL}"
